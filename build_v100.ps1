...
$logRx = New-Object System.Text.RegularExpressions.Regex('        public static string LogPath = Path.Combine\(Path.GetTempPath\(\), "EpohaShell-[^"]+\.log"\);')
$src = $logRx.Replace($src,'        public static string LogPath = Path.Combine(Path.GetTempPath(), "EpohaShell-1.0.0.log");`r`n        public static string WatchdogMarkerPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Epoha", "watchdog.enabled");',1)
...
$clubRuntime = @'
        ...
        private void ApplyWatchdog(bool enabled)
        {
            try
            {
                ...
                if(enabled){File.WriteAllText(Program.WatchdogMarkerPath,Application.ExecutablePath);if(!watchdogStarted){ProcessStartInfo pi=new ProcessStartInfo(Application.ExecutablePath,"--watchdog "+Process.GetCurrentProcess().Id.ToString());pi.UseShellExecute=false;pi.CreateNoWindow=true;Process.Start(pi);watchdogStarted=true;}}
