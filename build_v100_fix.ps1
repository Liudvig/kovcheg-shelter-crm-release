...
function Replace-Exact([string]$old,[string]$replacement,[string]$name) {
    if(-not $script:src.Contains($old)){ throw ("1.0.0 target missing: " + $name) }
    $script:src = $script:src.Replace($old,$replacement)
}

Replace-Exact '        public bool ClubModeEnabled = false;' "        public bool ClubModeEnabled = false;`r`n        public bool WatchdogEnabled = true;`r`n        public bool ReplaceExplorerShell = false;" 'config fields'
Replace-Exact '                c.ClubModeEnabled = BoolAttr(root, "clubModeEnabled", false);' "                c.ClubModeEnabled = BoolAttr(root, \"clubModeEnabled\", false);`r`n                c.WatchdogEnabled = BoolAttr(root, \"watchdogEnabled\", true);`r`n                c.ReplaceExplorerShell = BoolAttr(root, \"replaceExplorerShell\", false);" 'config load'
Replace-Exact '            root.SetAttribute("closeExplorerInClubMode", CloseExplorerInClubMode.ToString()); root.SetAttribute("clubModeEnabled", ClubModeEnabled.ToString());' "            root.SetAttribute(\"closeExplorerInClubMode\", CloseExplorerInClubMode.ToString()); root.SetAttribute(\"clubModeEnabled\", ClubModeEnabled.ToString());`r`n            root.SetAttribute(\"watchdogEnabled\", WatchdogEnabled.ToString()); root.SetAttribute(\"replaceExplorerShell\", ReplaceExplorerShell.ToString());" 'config save'

$programFields = @'
        public static string LogPath = Path.Combine(Path.GetTempPath(), "EpohaShell-1.0.0.log");
        public static string WatchdogMarkerPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Epoha", "watchdog.enabled");
        private static System.Threading.Mutex InstanceMutex;
'@
Replace-One '        public static string LogPath = Path.Combine\(Path.GetTempPath\(\), "EpohaShell-[^"]+\.log"\);' $programFields 'program fields'
...
$mainReplacement = @'
        ...
        public static void Main(string[] args)
        {
            ...
            bool created=true;
            try{InstanceMutex=new System.Threading.Mutex(true,"Local\\EpohaShell.Main",out created);}catch{created=true;}
            if(!created)return;
'@
        ...
        private static void RunWatchdog(string[] args)
        {
            try
            {
                ...
                for(int attempt=0;attempt<4&&File.Exists(WatchdogMarkerPath);attempt++)
                {
                    try
                    {
                        Process n=Process.Start(exe);if(n==null)return;
...
'@
Replace-Exact '        private static void Fatal(string where, Exception ex)' ($watchdogMethod + '        private static void Fatal(string where, Exception ex)') 'watchdog method'
Replace-Exact '        private IntPtr keyboardHook = IntPtr.Zero;' "        private IntPtr keyboardHook = IntPtr.Zero;`r`n        private bool watchdogStarted = false;" 'watchdog field'
...
$clubRuntime = @'
        private void SetClubMode(bool enabled)
        {
            ...
            cfg.ClubModeEnabled=enabled;cfg.Save();ApplyClubMode();if(!enabled)OpenExplorer();
        }

        private void ApplyClubMode()
        {
            try{ThemeWallpaper.HeaderTransparency=cfg.HeaderTransparency;SetTaskManagerPolicy(cfg.ClubModeEnabled&&cfg.BlockTaskManager);SetKeyboardHook(cfg.ClubModeEnabled&&cfg.BlockSystemKeys);ApplyUserShell(cfg.ClubModeEnabled&&cfg.ReplaceExplorerShell);ApplyWatchdog(cfg.ClubModeEnabled&&cfg.WatchdogEnabled);if(cfg.ClubModeEnabled&&cfg.CloseExplorerInClubMode)CloseExplorer();}
            catch(Exception ex){File.AppendAllText(Program.LogPath,"ClubMode: "+ex+"\r\n");}
        }

        private void ApplyWatchdog(bool enabled)
        {
            try{string dir=Path.GetDirectoryName(Program.WatchdogMarkerPath);if(!Directory.Exists(dir))Directory.CreateDirectory(dir);if(enabled){File.WriteAllText(Program.WatchdogMarkerPath,Application.ExecutablePath);if(!watchdogStarted){ProcessStartInfo pi=new ProcessStartInfo(Application.ExecutablePath,"--watchdog "+Process.GetCurrentProcess().Id.ToString());pi.UseShellExecute=false;pi.CreateNoWindow=true;Process.Start(pi);watchdogStarted=true;}}else if(File.Exists(Program.WatchdogMarkerPath))File.Delete(Program.WatchdogMarkerPath);}
        ...
        private void ApplyUserShell(bool enabled)
        {
            ...
            {
                if(enabled){if(Convert.ToInt32(state.GetValue("ShellBackupSaved",0))==0){object old=wk.GetValue("Shell",null);state.SetValue("ShellBackupSaved",1,RegistryValueKind.DWord);if(old==null)state.SetValue("ShellBackupEmpty",1,RegistryValueKind.DWord);else state.SetValue("ShellBackup",old.ToString(),RegistryValueKind.String);}wk.SetValue("Shell",Application.ExecutablePath,RegistryValueKind.String);}
                else{if(Convert.ToInt32(state.GetValue("ShellBackupSaved",0))!=0){if(Convert.ToInt32(state.GetValue("ShellBackupEmpty",0))!=0)wk.DeleteValue("Shell",false);else{object old=state.GetValue("ShellBackup",null);if(old!=null)wk.SetValue("Shell",old.ToString(),RegistryValueKind.String);}state.DeleteValue("ShellBackupSaved",false);state.DeleteValue("ShellBackupEmpty",false);state.DeleteValue("ShellBackup",false);}else{object cur=wk.GetValue("Shell",null);if(cur!=null&&String.Equals(cur.ToString(),Application.ExecutablePath,StringComparison.OrdinalIgnoreCase))wk.DeleteValue("Shell",false);}}
            }
        }

        private void SetTaskManagerPolicy(bool enabled)
        {
            using(RegistryKey k=Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Policies\System")){if(enabled){k.SetValue("DisableTaskMgr",1,RegistryValueKind.DWord);k.SetValue("DisableLockWorkstation",1,RegistryValueKind.DWord);k.SetValue("DisableChangePassword",1,RegistryValueKind.DWord);}else{k.DeleteValue("DisableTaskMgr",false);k.DeleteValue("DisableLockWorkstation",false);k.DeleteValue("DisableChangePassword",false);}}
            using(RegistryKey e=Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Policies\Explorer")){if(enabled)e.SetValue("NoLogoff",1,RegistryValueKind.DWord);else e.DeleteValue("NoLogoff",false);}
        }

        private void SetKeyboardHook(bool enabled){if(enabled&&keyboardHook==IntPtr.Zero){keyboardProc=new LowLevelKeyboardProc(KeyboardHookCallback);keyboardHook=SetWindowsHookEx(WH_KEYBOARD_LL,keyboardProc,GetModuleHandle(null),0);}else if(!enabled&&keyboardHook!=IntPtr.Zero){UnhookWindowsHookEx(keyboardHook);keyboardHook=IntPtr.Zero;keyboardProc=null;}}
        private IntPtr KeyboardHookCallback(int nCode,IntPtr wParam,IntPtr lParam){if(nCode>=0&&cfg.ClubModeEnabled&&cfg.BlockSystemKeys&&!adminMode){int vk=System.Runtime.InteropServices.Marshal.ReadInt32(lParam);bool alt=(Control.ModifierKeys&Keys.Alt)==Keys.Alt;bool ctrl=(Control.ModifierKeys&Keys.Control)==Keys.Control;if(vk==(int)Keys.LWin||vk==(int)Keys.RWin||(alt&&vk==(int)Keys.Tab)||(alt&&vk==(int)Keys.Escape)||(alt&&vk==(int)Keys.F4)||(ctrl&&vk==(int)Keys.Escape))return (IntPtr)1;}return CallNextHookEx(keyboardHook,nCode,wParam,lParam);}
        private void CloseExplorer(){try{Process[] ps=Process.GetProcessesByName("explorer");foreach(Process p in ps){try{p.Kill();}catch{}}}catch{}}
